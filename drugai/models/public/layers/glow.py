...
from .basic import ZeroConv2d, ActNorm, InvConv2dLU, InvConv2d, InvRotationLU, InvRotation, ActNorm2D
from .coupling import AffineCoupling, GraphAffineCoupling
...
class Flow(nn.Module):
    def __init__(self, in_channel, hidden_channels, affine=True, conv_lu=2, mask_swap=False):
        ...
        self.actnorm = ActNorm(in_channel)#把输入的各个通道归一化为0均值，单位方差的通道数据后，进行线性变换。
    ...
    def reverse(self, output):
        input = self.coupling.reverse(output)#[100, 36, 3, 3] ->
        if self.invconv:
...
class FlowOnGraph(nn.Module):
    ...
    def forward(self, adj, input):  # (2,4,9,9) (2,2,9,5)
        # if input are two channel identical, normalized results are 0 如果输入是两个相同的通道，则归一化结果为 0
        # change other normalization for input 更改输入的其他规范化
        out, logdet = self.actnorm(input)
        # out = input
...
class Block(nn.Module):
    ...
    def _squeeze(self, x):
        ...
        squeezed = x.view(b_size, n_channel, height // fold,  fold,  width // fold,  fold)#[100, 4, 3,3,3,3]
        squeezed = squeezed.permute(0, 1, 3, 5, 2, 4).contiguous()
        out = squeezed.view(b_size, n_channel * fold * fold, height // fold, width // fold)#[100, 36, 3, 3]
        return out
...
class Glow(nn.Module):
    ...
    def reverse(self, z):  # _list, reconstruct=False):
        h = z #[100, 4, 9, 9]
        for i, block in enumerate(self.blocks[::-1]):# 倒叙
            h = block.reverse(h)
