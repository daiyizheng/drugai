# -*- encoding: utf-8 -*-
'''
Filename         :coupling.py
Description      :
Time             :2022/08/04 00:45:51
Author           :daiyizheng
Email            :387942239@qq.com
Version          :1.0
Code come from https://github.com/calvin-zcx/moflow
'''
...
class AffineCoupling(nn.Module):  # delete
    def __init__(self,
                 ...
                 hidden_channels: int,
                 affine: bool = True,
                 mask_swap: bool = False 
                 )->None:  # filter_size=512,  --> hidden_channels =(512, 512)
        super(AffineCoupling, self).__init__()
        ...
        for h in vh:
            self.layers.append(nn.Conv2d(last_h, h, kernel_size=3, padding=1))
            # , momentum=0.9 may change norm later, where to use norm? for the residual? or the sum
    ...
    def forward(self, input):
        ...
        if self.affine:
            # log_s, t = self.net(in_a).chunk(2, 1)  # (2,12,32,32) --> (2,6,32,32), (2,6,32,32)
            s, t = self._s_t_function(in_a)
            # different affine bias , no difference to the log-det # (2,6,32,32) More stable, less error
            out_b = (in_b + t) * s
            # out_b = in_b * s + t
            logdet = torch.sum(
                torch.log(torch.abs(s)).view(input.shape[0], -1), 1)
        else:  # add coupling
            # net_out = self.net(in_a)
            _, t = self._s_t_function(in_a)
            out_b = in_b + t
...
class GraphAffineCoupling(nn.Module):
    def __init__(self, n_node, in_dim, hidden_dim_dict, masked_row, affine=True):
        ...
        # masked_row are kept same, and used for _s_t for updating the left rows masked_row 保持不变，并用于 _s_t 更新左侧行
        mask[masked_row, :] = 0
    ...
    def forward(self, adj, input):
        ...
        if self.affine:
            ...
            logdet = torch.sum(torch.log(torch.abs(s)).view(
                input.shape[0], -1), 1)  # possibly wrong answer 可能是错误的答案
        else:  # add coupling
    ...
    def _s_t_function(self, adj, x):
        ...
        if self.affine:
            ...
            #  x = sigmoid(log_x+bias): glow code Top 1 choice, keep s away from 0, s!!!!= 0  always safe!!! x = sigmoid(log_x+bias): 发光代码 Top 1 选择，让 s 远离 0，s!!!!= 0 总是安全的！！！
            # And get the signal from added noise in the  input 并从输入中添加的噪声中获取信号
...
def test_AffineCoupling():
    ...
    adj = torch.randint(0, 2, (bs, num_edge_type, nodes,
                        nodes), dtype=torch.float32)
    # adj = rescale_adj(adj)
...
def test_GraphAffineCoupling():
    ...
    gc = GraphAffineCoupling(
        nodes, in_dim, hidden_dim_dict, masked_row=range(0, nodes, 2), affine=True)
