...
class InvConv2dLU(nn.Module):
    def __init__(self, in_channel):
        super().__init__()
        # 考虑到权重W计算的复杂度，可利用LU分解为
        weight = np.random.randn(in_channel, in_channel)
        q, _ = la.qr(weight) # 计算矩阵的 QR 分解, 分解成一个正规正交矩阵Q与上三角形矩阵R
        w_p, w_l, w_u = la.lu(q.astype(np.float32))# LU分解，矩阵分解的一种，将系数矩阵A转为等价的两个矩阵L、U的乘积，L为单位下三角，U为上三角，是高斯消元法的一种表达式。在线性代数中已经证明，如果方阵是非奇异的，即的行列式不为0，LU分解总是存在的。如：A = P L U， 其中 P 是置换矩阵，L 是具有单位对角元素的下三角矩阵，U 是上三角矩阵。
        w_s = np.diag(w_u)# 以一维数组的形式返回方阵的对角线（或非对角线）元素
        w_u = np.triu(w_u, 1)
        u_mask = np.triu(np.ones_like(w_u), 1)#上三角矩阵
        l_mask = u_mask.T
        ...
        self.register_buffer('s_sign', torch.sign(w_s))# sign函数就是符号函数
...
class GraphConv(nn.Module):
    ...
    def forward(self, adj, h):
        ...
        # --- self connection, apply linear function 自连接，应用线性函数---
        hs = self.graph_linear_self(h)  # (256,9, 8) --> (256,9, 64)
        # --- relational feature, from neighbor connection  关系特征，来自邻居连接 ---
        # Expected number of neighbors of a vertex 顶点的预期邻居数
        # Since you have to divide by it, if its 0, you need to arbitrarily set it to 1 既然要除以它，如果它为0，则需要任意设置为1
