...
from drugai.models.public.layers.glow import Glow, GlowOnGraph

logger = logging.getLogger(__name__)


def gaussian_nll(x: torch.Tensor,
                 mean: torch.Tensor,
                 ln_var: torch.Tensor,
                 reduce: Text = 'sum'
                 ) -> torch.Tensor:
...
def rescale_adj(adj: torch.Tensor,
                type: Text = 'all'
                ) -> torch.Tensor:
...
def logit_pre_process(x: torch.Tensor,
                      a: float = 0.05,
                      bounds: float = 0.9
                      ) -> Tuple:
...
class MoFlow(nn.Module):
    def __init__(self,
                 ...
                 a_n_flow: int,
                 a_n_block: int,
                 mask_row_size_list: List,
                 mask_row_stride_list: List,
                 a_affine: bool
                 ) -> None:
        super(MoFlow, self).__init__()

        # More parameters derived from hyper_params for easy use
        self.b_n_type = b_n_type  # 健类型数
        self.a_n_node = a_n_node  # 原子类型数
    ...
    def forward(self,
        ...
        """
        h = x  # (256,9,5) # 原子图特征
        # add uniform noise to node feature matrices 向节点特征矩阵添加均匀噪声
        # + noise didn't change log-det. 1. change to logit transform 2. *0.9 ---> *other value??? 噪音没有改变log-det。 1. 更改为 logit 变换 2. *0.9 ---> *其他值？？？
        if self.training:
            if self.noise_scale == 0:
                h = h / 2.0 - 0.5 + torch.rand_like(x, dtype=torch.float) * 0.4  # / 2.0  similar to X + U(0, 0.8)   *0.5*0.8=0.4
            else:
                h = h + torch.rand_like(x, dtype=torch.float) * self.noise_scale  # noise_scale default 0.9
        ...
        if self.training:
            if self.noise_scale == 0:
                adj = adj / 2.0 - 0.5 + torch.rand_like(adj, dtype=torch.float) * 0.4  # / 2.0
            else:
                adj = adj + torch.rand_like(adj, dtype=torch.float) * self.noise_scale  # (256,4,9,9) noise_scale default 0.9
    ...
    def reverse(self,
                z: torch.Tensor,
                true_adj: torch.Tensor = None
                ) -> Tuple:  # change!!! z[0] --> for z_x, z[1] for z_adj, a list!!!
        """
    ...
    def log_prob(self,
                 z:torch.Tensor,
                 logdet:torch.Tensor
                 )->List:  # z:[(256,45), (256,324)] logdet:[(256,),(256,)]
